...
function startOrder(){cart=[];showView("orderView");renderDrinkPicker()}
...
function renderCheckout(){let s=cartSubtotal();$("orderContent").innerHTML=`<div class="panel"><h2>Delivery & Checkout</h2><div class="notice">Minimum: <strong>${money(MENU.business.deliveryMinimum)}</strong> before fees. Cash delivery is free; Venmo adds ${money(MENU.business.venmoDeliveryFee)}.</div>
<label>Name *</label><input id="custName"><label>Phone *</label><input id="custPhone" type="tel"><label>Email *</label><input id="custEmail" type="email" required><label>Delivery town *</label><select id="town"><option value="">Choose...</option>${Object.keys(MENU.business.areas).map(x=>`<option>${escapeHtml(x)}</option>`).join("")}</select><label>Delivery address *</label><input id="address"><label>Payment *</label><select id="payment"><option value="">Choose...</option><option>Cash</option><option>Venmo</option></select><label>Order notes</label><textarea id="orderNotes"></textarea>
<div class="summary"><div class="summary-row"><span>Subtotal</span><strong>${money(s)}</strong></div><div class="summary-row"><span>Delivery</span><strong id="deliveryFee">${money(0)}</strong></div><div class="summary-row total"><span>Total</span><strong id="checkoutTotal">${money(s)}</strong></div></div><div class="actions"><button onclick="renderCart()">← Back</button><button class="primary" onclick="submitOrder()">Place Order</button></div></div>`;$("payment").onchange=updateCheckoutTotal}
...
let p=$("payment").value,s=cartSubtotal(),email=$("custEmail").value.trim(),d={customer:{name:$("custName").value.trim(),phone:$("custPhone").value.trim(),email},delivery:{town:$("town").value,address:$("address").value.trim()},payment:p,notes:$("orderNotes").value.trim(),items:cart.map(i=>({sourceName:i.sourceName,name:i.name,size:i.size,price:i.price,itemPrice:i.itemPrice,milk:i.milk,flavors:i.flavors,bottom:i.bottom,espresso:i.espresso,lotus:i.lotus,coldFoam:i.coldFoam,notes:i.notes})),subtotal:s,deliveryFee:p==="Venmo"?MENU.business.venmoDeliveryFee:0,total:s+(p==="Venmo"?MENU.business.venmoDeliveryFee:0),submittedAt:new Date().toISOString()};

if(!d.customer.name||!d.customer.phone||!d.customer.email||!d.delivery.town||!d.delivery.address||!p){alert("Please complete all required fields, including email.");return}
...
function showConfirmation(d,n){$("orderContent").innerHTML=`<div class="panel"><h2>☕ Order Received!</h2><div class="notice"><strong>Order #${escapeHtml(n)}</strong></div><p>Thanks, ${escapeHtml(d.customer.name)}! We received your order.</p><div class="summary">${d.items.map(i=>`<div class="summary-row"><span>${escapeHtml(i.sourceName)}</span><strong>${money(i.itemPrice)}</strong></div>`).join("")}<div class="summary-row"><span>Delivery</span><span>${money(d.deliveryFee)}</span></div><div class="summary-row total"><span>Total</span><strong>${money(d.total)}</strong></div></div><p>Payment: <strong>${escapeHtml(d.payment)}</strong></p><div class="actions"><button onclick="goHome()">Done</button><button onclick="startOrder()">New Order</button></div></div>`;cart=[]}
...
function renderInfo(){$("infoContent").innerHTML=`<div class="panel"><h2>Delivery Information</h2><div class="notice">🚗 <strong>Minimum delivery order: ${money(5)}</strong> before delivery fees.</div><p><strong>Order cutoff:</strong> 5:00 AM the day of your order.</p><p><strong>Monday-Friday</strong></p><ul>${Object.entries(MENU.business.areas).map(([a,t])=>`<li><strong>${escapeHtml(a)}:</strong> ${escapeHtml(t)}</li>`).join("")}</ul><p>Frenchville & St. Agatha have a one-hour delivery window. Madawaska orders must be in at least 45 minutes before expected delivery.</p><p>Cash delivery is free; Venmo delivery is +$2. No Canada deliveries.</p><button class="primary" onclick="startOrder()">Start an Order</button></div>`}
function initChat(){
...
function answerChat(raw){let t=raw.toLowerCase();if(t.includes("minimum")||t.includes("$5"))return addBot("Our delivery minimum is $5.00 before delivery fees.");if(t.includes("hour")||t.includes("open")||t.includes("close"))return addBot("We deliver Monday-Friday. Frenchville and St. Agatha are generally 8:00-9:00 AM; Madawaska is generally 9:30 AM-12:00 PM. Orders are due by 5:00 AM.");if(t.includes("venmo"))return addBot("Cash delivery is free. Venmo delivery adds $2.00.");if(t.includes("canada"))return addBot("Sorry — we do not currently deliver to Canada.");if(t.includes("flavor"))return addBot(`We currently have ${MENU.flavors.filter(x=>x.available).length} available flavors. You can choose up to 4 flavors.`);if(t.includes("size"))return addBot("Cold Brews and Carnival Lemonade have Large and XL options; other drinks use the size listed on the menu.");if(t.includes("order")||t.includes("drink")||t.includes("coffee")){addBot("Absolutely — opening the order form now.");setTimeout(startOrder,350);return}if(t.includes("milk"))return addBot(`Available milk choices are ${MENU.milks.filter(x=>x.available).map(x=>milkLabel(x.name)).join(", ")}.`);addBot("I can help with the menu, flavors, milk choices, delivery, prices, or placing an order.")}
